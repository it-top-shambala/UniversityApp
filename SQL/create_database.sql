...
FROM table_marks
         ...
         JOIN view_teachers
              ON table_marks.student_id = view_teachers.id;

-- Наполнение таблиц тестовыми данными
INSERT INTO table_persons(last_name, first_name, patronymic, date_of_birth)
VALUES ('Starinin', 'Andrey', 'Nikolaevich', '1086-02-18'),
        ('Ivanov', 'Ivan', 'Ivanovich', '2000-01-01');

INSERT INTO table_subjects(subject)
VALUES ('Programming'), ('GUI');

INSERT INTO table_faculties(faculty)
VALUES ('SoftDev'), ('Design');

INSERT INTO table_teachers(person_id, faculty_id)
VALUES ((SELECT id FROM table_persons WHERE last_name = 'Starinin' AND first_name = 'Andrey'),
        (SELECT id FROM table_faculties WHERE faculty = 'SoftDev'));

INSERT INTO table_teacher_subjects(teacher_id, subject_id)
VALUES ((SELECT id FROM view_teachers WHERE full_name = 'Starinin Andrey Nikolaevich'), 
        (SELECT id FROM table_subjects WHERE subject = 'Programming')),
       ((SELECT id FROM view_teachers WHERE full_name = 'Starinin Andrey Nikolaevich'),
        (SELECT id FROM table_subjects WHERE subject = 'GUI'));

INSERT INTO table_students(person_id, faculty_id, course)
VALUES ((SELECT id FROM table_persons WHERE last_name = 'Ivanov' AND first_name = 'Ivan'),
        (SELECT id FROM table_faculties WHERE faculty = 'SoftDev'), 1);

INSERT INTO table_marks(student_id, mark, subject_id, teacher_id)
VALUES ((SELECT id FROM view_students WHERE full_name = 'Ivanov Ivan Ivanovich'), 3, 
        (SELECT id FROM table_subjects WHERE subject = 'Programming'), 
        (SELECT id FROM view_teachers WHERE full_name = 'Starinin Andrey Nikolaevich'));
INSERT INTO table_marks(student_id, date, mark, subject_id, teacher_id)
VALUES ((SELECT id FROM view_students WHERE full_name = 'Ivanov Ivan Ivanovich'), 
        '2023-10-06', 4,
        (SELECT id FROM table_subjects WHERE subject = 'Programming'),
        (SELECT id FROM view_teachers WHERE full_name = 'Starinin Andrey Nikolaevich'));
INSERT INTO table_marks(student_id, mark, subject_id, teacher_id)
VALUES ((SELECT id FROM view_students WHERE full_name = 'Ivanov Ivan Ivanovich'), 5,
        (SELECT id FROM table_subjects WHERE subject = 'GUI'),
        (SELECT id FROM view_teachers WHERE full_name = 'Starinin Andrey Nikolaevich'));